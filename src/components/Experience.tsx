...
const timelineData: Milestone[] = [
  {
    ...
    icon: Code2,
    milestones: [
      "Learned modern frontend development using HTML, CSS, JavaScript, and React.js.",
      "Built responsive web interfaces and reusable components through hands-on projects.",
      "Gained experience with API integration, Git/GitHub, debugging, and frontend best practices.",
    ],
  ...
  },
  {
    ...
    milestones: [
      ...
      "Built cross-platform mobile applications with React Native and integrated REST APIs.",
      "Collaborated with cross-functional teams to deliver high-quality, responsive, and user-friendly solutions.",
    ],
    tech: ["ReactJS", "Angular", "TypeScript", "React Native", "REST APIs"],
  },
];
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20">
          ...
          >
            A scroll-driven journey detailing how my technical role,
            architectural thinking, and code stack matured year-by-year.
          </motion.p>
        </div>
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-px bg-card-border transform -translate-x-1/2" />
            ...
            {timelineData.map((item, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`w-full md:w-1/2 pl-12 md:pl-0 md:pr-12 md:text-right ${isEven ? "md:order-1" : "md:order-3"}`}
                  >
                    <motion.div
                    ...
                    >
                      {/* Date Badge */}
                      <div
                        className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-primary/10 text-primary mb-4 ${isEven ? "md:flex-row-reverse" : ""}`}
                      >
                      ...
                      {/* Milestones list */}
                      <ul
                        className={`mt-5 space-y-3.5 text-xs text-foreground/75 ${isEven ? "md:text-right" : "md:text-left"}`}
                      >
                        {item.milestones.map((mil, milIdx) => (
                          <li
                            key={milIdx}
                            className={`flex items-start gap-2.5 ${isEven ? "md:flex-row-reverse" : ""}`}
                          >
                      ...
                      {/* Technical Capsules */}
                      <div
                        className={`flex flex-wrap gap-1.5 mt-6 ${isEven ? "md:justify-end" : "justify-start"}`}
                      >
                        {item.tech.map((t) => (
                          <span
                            key={t}
                            className="text-[9px] font-bold bg-card-border/60 dark:bg-card-border/30 border border-card-border/80 px-2 py-0.5 rounded-md text-foreground/80"
                          >
                            {t}
                          </span>
                        ))}

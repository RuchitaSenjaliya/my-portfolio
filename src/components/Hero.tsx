...
import { Github, Linkedin, Instagram, Mail, ArrowRight, Download, Terminal, Code2 } from 'lucide-react';

const words = ['Frontend Developer', 'React Developer', 'React Native Developer'];
...
function CodeSnippet() {
  ...
  return (
    <div className="w-full max-w-md bg-slate-900 border border-slate-700/80 rounded-2xl shadow-2xl overflow-hidden font-mono text-[10px] sm:text-xs text-left text-blue-300">
      ...
      {/* Code window */}
      <div className="p-5 min-h-[160px] relative font-mono leading-relaxed bg-slate-950/40">
        <pre className="text-white/90 whitespace-pre-wrap">
          {text.split('\n').map((line, idx) => {
            // Basic syntax highlighting on output lines
            let formattedLine = line;
            if (line.includes('const')) {
              formattedLine = line.replace('const', '<span class="text-pink-500">const</span>');
            }
            if (line.includes('developer')) {
              formattedLine = formattedLine.replace('developer', '<span class="text-blue-400">developer</span>');
            }
            if (line.includes('name:')) {
              formattedLine = formattedLine.replace('name:', '<span class="text-purple-400">name</span>:');
            }
            if (line.includes('skills:')) {
              formattedLine = formattedLine.replace('skills:', '<span class="text-purple-400">skills</span>:');
            }
            if (line.includes('creativity:')) {
              formattedLine = formattedLine.replace('creativity:', '<span class="text-purple-400">creativity</span>:');
            }
            if (line.includes('available:')) {
              formattedLine = formattedLine.replace('available:', '<span class="text-purple-400">available</span>:');
            }
            if (line.includes('code:')) {
              formattedLine = formattedLine.replace('code:', '<span class="text-purple-400">code</span>:');
            }
            if (line.includes('true')) {
              formattedLine = formattedLine.replace('true', '<span class="text-amber-500">true</span>');
            }
            
            return (
              <div key={idx} className="flex gap-4">
                <span className="w-4 text-white/25 text-right select-none">{idx + 1}</span>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 flex flex-col items-center lg:items-start text-center lg:text-left">
            ...
            >
              Hi, I am <span className="bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">Ruchita Senjaliya</span>
            </motion.h1>
            ...
            >
              <button
                ...
                className="flex items-center justify-center gap-2 px-6 py-3.5 bg-gradient-to-r from-primary to-secondary text-white font-semibold rounded-full hover:shadow-lg hover:shadow-primary/25 active:scale-98 transition-all duration-200 cursor-pointer text-xs"
              >
